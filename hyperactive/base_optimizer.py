...
class BaseOptimizer:
    ...
    def search(self, nth_process, X, y):
        ...
        for i in range(self._config_.n_iter):
            ...
            _cand_ = self._iterate(i, _cand_, _p_, X, y,run)
            self._config_.update_p_bar(1)
            t2 = time.time()
            print("score_run:"+str(run), _cand_.score_best)
            print("time:", t2-t1)            
            output = pd.DataFrame({"run":[run],"iter":[i+1],"score":[_cand_.score_best],"time":[t2-t1]})
            output.to_csv(os.path.join("output", "pso_apnea_best_iter.csv"), mode='a', index=False,header=False)
